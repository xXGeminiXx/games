...
export const CONTENT = {
  ...
  ui: {
    ...
    treeFell: 'A grown one is worth {felled} sugar felled. Left standing it trades that much back in {time}.',
    treeFellIdle: 'A grown one is worth {felled} sugar felled, and it is trading nothing to set against that.',
    sentNothingAuto: '{Name} pays less than the others right now, so nothing goes to it. It gets minerals again the moment it pays better.',
    sentNothing: 'You are sending nothing here, so it earns nothing. Give it a share to start it trading.',
    treeGone: 'Nothing of this kind left standing. The wood is still there to eat, and a seedling comes up after it.',
    treeFeed: 'Feeding pays for itself in {time}.',
    treeFeedNever: 'Feeding wouldn\'t pay for itself here yet.',
    treeFeedWinter: 'Nothing grows in winter, so feeding buys nothing until spring.',
    ...
    next: {
      hand:     'Each press eats a little of the log and pushes the thread out. {n} more presses and you can afford the first tip.',
      handLast: 'One more press buys the first tip. A tip forages on its own and never stops.',
      firstTip: 'A tip forages on its own and never stops. You can afford one, so stop pressing.',
