...
import { fullscreen, target, bindScreen, program, buffer, vao } from './gl.js?v=2';
import { orbitCamera } from './camera.js?v=2';
import { m4, clamp } from './mat.js?v=2';
import { hexToRgb } from './palette.js?v=2';
import { carrierMesh } from './carrier.js?v=2';
...
export function createView(G, canvas, cfg, S, world, drones) {
  ...
  // Nothing may be anchored past the edge of the world.
  const limit = () => cfg.world.size * 0.46;
  const clampAnchor = (x, z) => { const l = limit(); return [Math.max(-l, Math.min(l, x)), Math.max(-l, Math.min(l, z))]; };
  ...
  let sceneScale = 0;
  const setScale = (s) => { cfg.render.scale = s; scene = null; };

  const draw = (t, range, fogK = 0.0009) => {
    if (G.resize(1) || !scene || sceneScale !== cfg.render.scale) {
      sceneScale = cfg.render.scale;
      scene = target(gl, { width: Math.max(2, Math.floor(canvas.width * sceneScale)), height: Math.max(2, Math.floor(canvas.height * sceneScale)), format: 'rgba16f', filter: 'linear', depth: true });
    }
