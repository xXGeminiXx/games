...
export function drawWork(ctx, iso, cfg, glyph, sx, sy, size, angle, tier, lineColor) {
  ...
  // A hard little shadow on the ground, thrown to the lower right away from
  // the sun, so the work stands on the cell instead of floating over it. A
  // ghost being placed casts none.
  if (!lineColor && cfg.render.workShadow > 0) {
    ctx.globalAlpha = cfg.render.workShadow;
    ctx.fillStyle = cfg.render.edge;
    ctx.beginPath();
    const rx = body * 0.5, ry = body * 0.22;
    const ox = cx + body * 0.28, oy = foot + body * 0.1;
    ctx.moveTo(ox + rx, oy);
    ctx.lineTo(ox + rx * 0.45, oy + ry);
    ctx.lineTo(ox - rx * 0.45, oy + ry);
    ctx.lineTo(ox - rx, oy);
    ctx.lineTo(ox - rx * 0.45, oy - ry);
    ctx.lineTo(ox + rx * 0.45, oy - ry);
