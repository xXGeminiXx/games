...
import { dominantTrait, TRAIT, TRAIT_IDS } from '../traits.js?v=4';
...
export function createScene(cfg, canvas, iso, ground) {
  ...
  function drawFallRow(terrain, line, d) {
   ...
   */
  function drawBranches(terrain, lines) {
  ...
  const scene = {
    ...
    draw(state, dt) {
      ...
      for (let d = 0; d < rows; d++) {
        ...
        if (haveLine) drawFallRow(terrain, line, d);
      ...
      drawBranches(terrain, state.fallLines);
